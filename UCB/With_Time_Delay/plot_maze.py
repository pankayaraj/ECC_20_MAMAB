...
for iter in range(a,b):
    ...
    ax5 = fig.add_subplot(gs[0:1,0:2])
    for ag in range(agents):

        k = choice[n][ag][iter][1] #indices prob, agent, iter, nextban
    ...
    ax3_self.plot((1/no_agents)*np.sum(self_total[n][:,0:iter+1], axis=0))
    ...
    ax4_mean.bar([_ for _ in range(no_bandits)], mean)
    for ag in range(agents):
        if choice[n][ag][iter][0] != None:
            pick = choice[n][ag][iter][0]
    ...
    ax5.plot((1/no_agents)*np.sum(com_total[n][:,0:iter+1], axis=0))
    ax5.set_title("Agent's expected Cumulative Comminication regret")
    ax5.set_xlabel("Time")
    ax5.set_ylabel("Regret")

    plt.tight_layout()
    filename = "Maze_Images/maze" + str(iter) + ".png"
