...
for i in range(no_agents):
    Agent_Set_In.append(np.random.randint(0, len(Sets)))

Agent_X_ini = [[[np.random.normal(0, 1)*(1/no_agents)*12 for i in range(no_bandits)] for j in range(no_agents)] for _ in range(no_agents)]
N__ = [16]
#for no in range(0, 20, 4):
for no in N__:
#for no in range(16, 17):
    #if no == 0 or no == 4:
    #    continue
    print(no)
    ...
    for experiment  in range(1, 5):
        ...
        G = MAMAB(no_bandits=no_bandits, no_agents=no_agents, bandits=bandits,optimal_bandit_index=o ,
                  Sets=Sets, Ban_to_Set_dict=Ban_to_Set_dict, reward_vairance=variance, delta=delta,
                no_iter=no_iterations, agent_X_ini=Agent_X_ini, agent_set_in=Agent_Set_In,  n_j=no)
    ...
    np.save("Agent/Total/Agent_choice" + str(no), choice)
    ...
    #np.save("Global/Total/Global_tot_regret" + str(no), GLOBAL_TOT_REGRET)
